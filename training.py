...
from src.models.cvae_models import VAE
import time
...
for epoch in range(epochs):
    ...
    for protein, smiles, affinity, node_drug, edge_drug, batch, node_proteins, edge_proteins \
            in dataloader_train:
        end_time = time.time()
        ...
        if use_cuda:
            ...
            node_drug = node_drug.cuda()
            edge_drug = edge_drug.cuda()
            affinity = affinity.cuda()
            batch = batch.cuda()
            node_proteins = [p.cuda() for p in node_proteins]
            edge_proteins = [p.cuda() for p in edge_proteins]

        _, out = model(protein, smiles, node_drug, edge_drug, batch, vae, node_proteins, edge_proteins)
        ...
        print('# [{}/{}] training {:.1%} loss={:.5f}, ci={:.5f}'.format(epoch+1
                                                                    , epochs
                                                                    , b/train_epoch_size
                                                                    , loss
                                                                    , c_index
