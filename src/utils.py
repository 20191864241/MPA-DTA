...
class DrugTargetDataset(torch.utils.data.Dataset):
    def __init__(self, X0, X1, Y, pid, is_target_pretrain=True, is_drug_pretrain=False, self_link=True,
        ...
        smiles = DTAData(smiles)  # 转化为smiles_fingerprint
        ...
        self.z = z['dict'][()]
        node_proteins = np.load('data/node/' + str(dataset) + '_node.npz', allow_pickle=True)
        self.node_proteins = node_proteins['dict'][()]
        edge_proteins = np.load('data/edge/' + str(dataset) + '_edge.npz', allow_pickle=True)
    ...
    def __getitem__(self, i):
        ...
        node_proteins = torch.from_numpy(self.node_proteins[self.pid[i]]).squeeze().float()
        edge_proteins = torch.from_numpy(self.edge_proteins[self.pid[i]]).squeeze()
        return [prot, self.X2[i], self.Y[i], node_drugs, edge_drugs, node_proteins, edge_proteins]
...
def collate(args):
    ...
    z0 = [a[5] for a in args]
    z1 = [a[6] for a in args]

    batch = []
    cumulative_nodes = 0
    for idx, nodes in enumerate(node_drugs):
        batch.extend([idx] * nodes.size(0))
        if idx > 0:
            edge_drugs[idx] += cumulative_nodes
        cumulative_nodes += nodes.size(0)
    return x0, x1, torch.stack(y, 0), torch.cat(node_drugs, dim=0), \
           torch.cat(edge_drugs, dim=1), torch.tensor(batch, dtype=torch.long), z0, z1
